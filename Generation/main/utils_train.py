...
def get_trainer_callbacks(lightning_config, config, logdir, ckptdir, logger):
    default_callbacks_cfg = {
        ...
        "cuda_callback": {"target": "callbacks.CUDACallback"},
    }
...
def load_checkpoints(model, model_cfg):
    if check_config_attribute(model_cfg, "pretrained_checkpoint"):
        ...
        def load_partial_tensor(current_tensor, checkpoint_tensor):
            current_shape = current_tensor.shape
            checkpoint_shape = checkpoint_tensor.shape

            new_tensor = torch.zeros_like(current_tensor)

            slices = tuple(
                slice(0, min(c, p)) for c, p in zip(current_shape, checkpoint_shape)
            )

            new_tensor[slices] = checkpoint_tensor[slices]
        ...
        try:
            if "state_dict" in pl_sd.keys():
                all_state_dict = pl_sd["state_dict"]
                    ...
                    if k in current_state_dict:
                        if current_state_dict[k].shape != v.shape:
                            mismatched_params.append(
                                {
                                    "name": k,
                                    "checkpoint_shape": list(v.shape),
                                    "model_shape": list(current_state_dict[k].shape),
                                }
                            )
                            processed_state_dict[k] = load_partial_tensor(
                ...
                if mismatched_params:
                    mainlogger.info("\n=== Found Shape Mismatches ===")
                    for param in mismatched_params:
                        mainlogger.info(f"Parameter: {param['name']}")
                        mainlogger.info(
                            f"  Checkpoint shape: {param['checkpoint_shape']}"
                        )
                        mainlogger.info(f"  Model shape    : {param['model_shape']}")
                    mainlogger.info("===========================\n")

                model.load_state_dict(processed_state_dict, strict=False)
                mainlogger.info(
                    ">>> Loaded weights from pretrained checkpoint (excluding first_stage_model): %s"
                    % pretrained_ckpt
                )
            else:
                # deepspeed
                ...
                for key in pl_sd["module"].keys():
                    model_key = key[16:] if key.startswith("module.") else key
                    if model_key.startswith("first_stage_model"):
                        continue

                    checkpoint_tensor = pl_sd["module"][key]

                    if model_key in current_state_dict:
                        if (
                            current_state_dict[model_key].shape
                            ...
                            new_pl_sd[model_key] = load_partial_tensor(
                                current_state_dict[model_key], checkpoint_tensor
                            )
                        else:
                            new_pl_sd[model_key] = checkpoint_tensor
                ...
                mainlogger.info(
                    ">>> Loaded weights from pretrained checkpoint (excluding first_stage_model)"
                )
        except Exception as e:
            mainlogger.error(f"Error loading checkpoint: {e}")
            raise
    else:
