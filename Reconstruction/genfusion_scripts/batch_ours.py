...
base_dir = "./data"
experiment_name = "E4_ours_with_mono_depth"
...
def run_task(gpu_id, hash_id):
    ...
    with gpu_locks[gpu_id]:
        ...
        data_dir = f"{base_dir}/{hash_id}/nerfstudio"
        ...
        train_cmd = (
            ...
            f"--num_frames 16 "
            f"--outpaint_type crop "
            f"--add_indices 7 15 "
            f"--start_diffusion_iter 3000 "
            f"--depth_loss "
            f"--sparse_view 0 "
            ...
            f"--patch_size 480 270 "
            f"--opacity_reset_interval 9000 "
            f"--lambda_dist 0.0 "
            f"--lambda_reg 0.5 "
            f"--lambda_dssim 0.8 "
            f"--densify_from_iter 1000 "
            f"--unconditional_guidance_scale 3.2 "
            f"--repair"
        )
        train_cmd += f" --port {4050 + gpu_id}"
...
def main():
    ...
    if not hash_ids:
        logger.error(f"No valid hash directories found in {base_dir}")
        sys.exit(1)

    logger.info(f"Found {len(hash_ids)} hash directories to process")
